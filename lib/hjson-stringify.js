...
export function stringify(data, opt) {
  ...
  let commonRange='\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff';
  // needsEscape tests if the string can be written without escapes
  let needsEscape = new RegExp('[\\\\\\"\x00-\x1f'+commonRange+']', 'g');
  // needsQuotes tests if the string can be written as a quoteless string (like needsEscape but without \\ and \")
  let needsQuotes = new RegExp('^\\s|^"|^\'|^#|^\\/\\*|^\\/\\/|^\\{|^\\}|^\\[|^\\]|^:|^,|\\s$|[\x00-\x1f'+commonRange+']', 'g');
  ...
  let needsEscapeML = new RegExp('\'\'\'|^[\\s]+$|[\x00-'+(multiline === 2 ? '\x09' : '\x08')+'\x0b\x0c\x0e-\x1f'+commonRange+']', 'g');
  // starts with a keyword and optionally is followed by a comment
  let startsWithKeyword = new RegExp('^(true|false|null)\\s*((,|\\]|\\}|#|//|/\\*).*)?$');
  let meta = {
  ...
  function str(value, hasComment, noIndent, isRootObject) {
    ...
    switch (typeof value) {
      ...
      case 'object':
        ...
        let comments; // whitespace & comments
        ...
        if (isArray) {
          ...
          let commentKeys = comments ? comments.o.slice() : [];
          let objectKeys = [];
          for (k in value) {
  ...
  let res = "";
  let comments = keepComments ? comments = (common.getComment(data) || {}).r : null;
